...
def _extract_largest_cross_section(fbx_path: str) -> Polygon:
    """
    Load FBX and return its largest cross-sectional Shapely Polygon.
    Projects all faces onto each of the 3 axis-aligned planes, unions them,
    and returns whichever projection has the greatest area.
    """
    ...
    return translate(best_poly, xoff=-c.x, yoff=-c.y)
...
def _fit_poly_in_poly(bounding_poly: Polygon,
    ...
    def objective(params):
        ...
        overflow = max(0, temp.area - covered)
        # Objective: maximize coverage, but penalize overflow
        # Weighting: 10x on coverage, 1x on overflow allows overflow if coverage improves
        return -covered * 10.0 + overflow * 1.0

    best_res = None
    best_val = float('inf')

    # Try a wider range of starting scales to find better fits
    for s0 in [1.0, 0.8, 0.6, 0.4, 0.2, 0.1, 0.05]:
...
def depth_fit_nms_to_json(
    ...
    target = convert_mesh(target_mesh, "obj")
    ...
    for fbx_path in part_fbx_paths:
        ...
        cross_poly   = _extract_largest_cross_section(fbx_path)
        ...
        print(f"[depth_fit]   cross_area={cross_poly.area:.4f}  "
              f"cross_size={cross_size:.4f}  part_max_dim={part_max_dim:.4f}")
    ...
    for face_idx, face in enumerate(target.faces):
        ...
        face_area = remaining.area  # For threshold comparisons
        parts_placed_this_face = 0  # Track for debugging

        # -------------------------------------------------------------- #
        # 4. Depth loop: pick best part, place it, subtract, repeat
        # -------------------------------------------------------------- #
        for d in range(depth):
            # Handle MultiPolygon: convert to list of individual polygons
            if remaining.is_empty or remaining.area < 1e-10:
                break
            ...
            # Filter out empty/tiny polygons and find the largest one
            remaining_polys = [p for p in remaining_polys if p.area > 1e-10]
            if not remaining_polys:
                break
            
            # Focus optimization on the largest polygon to save time
            largest_idx = max(range(len(remaining_polys)), key=lambda i: remaining_polys[i].area)
            rem_poly = remaining_polys[largest_idx]
            ...
            # Try to fit each part into the largest remaining polygon
            for part in parts:
                score, s, theta, tx, ty = _fit_poly_in_poly(
                    rem_poly, part["cross_poly"]
                )
                if score > best_score:
                    best_score  = score
            ...
            # Use a threshold to stop trying if coverage is too small
            # Change to 0.1% of remaining area (not original face) to allow fitting in fragments
            min_threshold = rem_poly.area * 0.001 if rem_poly.area > 0 else 1e-6
            if best_part is None or best_score < min_threshold:
            ...
            up_vec = normal * scale_factor

            # At = in-plane forward, rotated by the optimizer's theta
            theta_rad = np.radians(theta)
            at_vec    = np.cos(theta_rad) * u + np.sin(theta_rad) * v
            ...
            # Subtract placed shape from the specific polygon it was placed in
            updated_poly = rem_poly.difference(best_placed_poly)
            if not updated_poly.is_valid:
                updated_poly = updated_poly.buffer(0)
            
            # Rebuild remaining: replace the largest polygon with its updated version
            remaining_polys[largest_idx] = updated_poly
            remaining = unary_union(remaining_polys) if remaining_polys else Polygon()
